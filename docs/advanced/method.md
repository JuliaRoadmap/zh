...
!!! note
    本章中的所有示例都假定是为*相同*模块中的函数定义模块。 如果你想给*另一个*模块中的函数添加方法，你必须`import`它或使用模块名称限定的名称。 请参阅有关 [命名空间管理](@todo) 的部分。
...
这样的定义对应着那些类型签名是 `UnionAll` 类型的方法（参见 [UnionAll 类型](typesystem.md#unionall)）
