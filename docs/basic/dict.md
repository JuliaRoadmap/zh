# 字典
使用 `Dict` 可以创建一个**字典**（dictionary），这是一个**键**（key）到**值**（value）映射的结构，可以按照键名存放或取出数据。例如，你可以用它存储一个英文单词对应的翻译（这可能键的类型是字符串，值的类型是字符串数组）。
...
通过对 `Dict` 遍历得到的 `Pair` 实例称为「键值对」，其中 `pair.first` 是键，`pair.second` 是值。
