# 浮点数与实数运算
浮点数是遵循 IEEE 754 标准的数据类型，广泛用于数学运算，用于表示实数的近似值。
以下是 Julia 预定义的浮点数类型：
...
不同于整数，并不存在表示“非负实数”的类型，因为这是完全不必要的。

## 浮点数字面表示
* 使用标准格式表示，例如 `1.234`，`1.3e8`（科学记数法），`.234`（省略 0）
* 也支持十六进制浮点字面量，规则较为特殊，详见[官方文档](https://docs.juliacn.com/latest/manual/integers-and-floating-point-numbers/#十六进制浮点数字面量)
* 也可以用 `_` 间隔数字以提高可读性，如 `1_000_000.0`
...
另外需要注意的是，在浮点数标准中 `-0.0` 与 `0.0` 是不同的：
```julia-repl
julia> -0.0
-0.0
