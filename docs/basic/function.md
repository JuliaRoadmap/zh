# 函数
**函数**（function）是可重复调用的代码块，通过关键字 `function` 定义。在 Julia 中，`Function` 是所有函数类型的超类型。
...
最后的 `return` 可以省略，但显式写出有助于阅读。若不写返回值，则返回 [`nothing`](little_types.md#无)。
...
* **派发** 如 [方法](../advanced/method.md) 中所述，对于不同的参数类型，你可以有实现不同的函数（称为“方法”），在这种情况下，参数类型用于确定调用哪个版本的函数
