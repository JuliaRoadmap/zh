# 变量简介
Julia 有丰富的**变量**（variable）类型，包括但不限于整数、浮点数、字符串、指针。它是一种动态类型语言，允许同一变量在不同时刻绑定不同类型的值。

如果你是新手，可以把“变量”理解为：使用一个名称（称为“变量名”）与一个值进行绑定。而这个变量名所代表的值是可变的，即在一段代码前后绑定的值可以不同。

可以在 REPL 中尝试以下示例：

下面演示变量的**初始化**（initialize）与**赋值**（assign）：
```julia-repl
julia> v = 1
1
```
...
最后的 `v += 1` 是 `v = v + 1` 的简写。注意这并不是数学错误，因为 `=` 的意思是赋值而不是表达左右两边相等。
...
julia> 变量(4) # 计算了 4 的平方根
2.0
```

在上述例子中，代码 `v = 1` 也可写作无空格 `v=1` 甚至使用制表符而不是空格的 `v=	1`. 在使用 `=`，`+` 这样的语法符号时，空格的多少对于程序正确运行无关紧要，只会影响美观性。为了提升代码美观性采取的选择称为“风格”。

!!! tips
	Julia 暂时不支持变量删除，如果有某个变量希望删除，可以赋值为 [nothing](little_types.md#无)，之后原本占据的内存会在某个时刻被回收（也可以调用 `GC.gc()` 主动触发回收，但一般不需要这么做）。

变量具有作用域。即：一个变量可能只在代码的一个特定区域可以访问。例如：
```julia-repl
julia> begin
       local new_var = "str"
       print(new_var)
...
str
julia> print(new_var)
ERROR: UndefVarError: `new_var` not defined in `Main`
```
...
利用[元组](./little_types.md#元组)可以同时显示多个变量的值：
```julia-repl
julia> a
0
