# 命令行
**命令行**（cli/command-line）通常指操作系统或一些程序中的命令式管理器，它通常显式地通过控制台（console）或终端（terminal）方便用户控制。
...
Julia 提供了 [`Pipe`](../advanced/io.md) 便于操作；在命令行中可使用 `A | B`。
这会在内存中创建一个管道，并同时启动两个程序。A 的输出会被写入该管道；由于管道缓冲区很小，在 B 尚未读取时，A 的写入会暂时阻塞，等 B 读走数据后再继续。这样就避免了中间文件和额外磁盘 IO，也能让两段程序并发执行，同时减少中间结果的内存占用。
